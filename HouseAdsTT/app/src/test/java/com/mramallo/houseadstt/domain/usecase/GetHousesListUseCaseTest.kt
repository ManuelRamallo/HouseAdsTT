...
class GetHousesListUseCaseTest {

    @RelaxedMockK
    private lateinit var housesRepository: HousesRepository

    lateinit var getHousesListUseCase: GetHousesListUseCase

    @Before
    fun onBefore() {
        MockKAnnotations.init(this)
        getHousesListUseCase = GetHousesListUseCase(housesRepository)
    }

    @Test
    fun `when the api return something then get values from api`() = runBlocking {
        val houseList = mutableListOf<HouseItemDto>()
        houseList.add(getHouseItemDtoMock())
        coEvery { housesRepository.getAllHouses() } returns houseList

        val response = getHousesListUseCase()

        assert(houseList.toHousesList() == response)


    }


    private fun getHouseItemDtoMock(): HouseItemDto {
        return HouseItemDto(
            propertyCode = "1",
